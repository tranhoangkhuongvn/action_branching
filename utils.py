...
def parse_arguments():
	...
	parser.add_argument('configuration', type=str, help='the name of the configuration file (JSON)')
...
def _save(agent, rewards, env_name, output_dir, model_type):
	...
	plt.title('Branching DQN ({}) : {}'.format(agent.td_target, env_name))
...
def save_frames_as_gif(frames, output_dir, episode, dpi):
	...
	ani = animation.FuncAnimation(plt.gcf(), animate, frames=len(frames), interval=10)
