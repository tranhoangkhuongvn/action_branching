...
class BranchingDQN(nn.Module):
	...
	def update_policy(self, batch):
		...
		actions = torch.tensor(batch_actions).long().reshape(states.shape[0], -1, 1).to(self.device) #? why this shape
		...
		with torch.no_grad():
			...
			if self.td_target == 'mean':
				...
				max_next_Q = max_next_Q.max(1, keepdim=True)
		...
		if self.update_counter % self.target_update_freq == 0:
			self.update_counter = 0
			self.target_network.load_state_dict(self.policy_network.state_dict()) # hard update
			# TODO: maybe add in soft-update	
