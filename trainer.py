...
class Trainer:
	def __init__(self, model, env, memory, max_steps, max_episodes, 
				...
				start_learning, batch_size, save_update_freq, output_dir):
	...
	def loop(self):
		...
		for step in range(self.max_steps):
			epsilon = self._exploration(step) # ? why decay epsilon within an episode?
			...
			if done:
				...
				print('Episode: {} - Reward: {}'.format(len(all_rewards), episode_reward))
